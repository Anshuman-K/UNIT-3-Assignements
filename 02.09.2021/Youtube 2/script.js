...
async function showTopLocationYoutube(){
    if(count>0){
        remove_predata();
    }
    let res = await fetch(`https://youtube.googleapis.com/youtube/v3/videos?chart=mostPopular&type=video&locale=India&regionCode=In&key=AIzaSyBr6cJLsDCjHkGOArtGkad0T7hUU_VZARw&maxResults=20`);
    let data = await res.json();
    console.log('data:', data);
    console.log("Hello");
